...
#        radmasses.append(float(f.replace("CMS_jj_","").split("_")[0])/1000.)
        #radmasses = [260, 270, 300, 350, 400, 450] 
        radmasses = [ 400, 450, 500, 550, 600, 650, 750, 800, 900, 1000, 1200]
    print radmasses

    efficiencies={}
    for mass in radmasses:
        efficiencies[mass]=100. # to convert from pb to fb
    ...
    if "qW" in label.split("_")[0] or "qZ" in label.split("_")[0]:
        mg.GetXaxis().SetLimits(260,1400)
    else:
        mg.GetXaxis().SetLimits(260, 1400)
	...
	#gtheory.SetPoint(ij, float(split[0][-4:])/1000., float(split[1])*0.57*0.57*1000.)
        ...
        gtheory.SetPoint(ij, float(split[0][-4:]), float(split[1]))
	print(float(split[1]))
        glogtheory.SetPoint(ij, float(split[0][-4:]), log(float(split[1])))
	ij+=1
    mg.Add(gtheory,"C")
    gtheory.Draw("C")
...
if __name__ == '__main__':
  ...
  for chan in channels:
    print "chan =",chan
    masses =[400, 450, 500, 550, 600, 650, 750, 800, 900, 1000, 1200]

    HPplots=[]
    LPplots=[]
    combinedplots=[]
    for mass in masses:
       HPplots+=["CMS_"+str(mass)+"_HH4b_13TeV_asymptoticCLs.root"]
